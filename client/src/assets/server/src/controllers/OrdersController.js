...
module.exports = {
  ...
  async show (req, res) {
    try{
      ...
      const rocks = await order.getRocks({attributes: ['id', 'name'], joinTableAttributes: []})
      const plants = await order.getPlants({attributes: ['id', 'name'], joinTableAttributes: []})
      const fishes = await order.getFishes({attributes: ['id', 'name'], joinTableAttributes: []})
      res.status(200).send({
        order: order,
        rocks: rocks,
        plants: plants,
